...
from transformers import CLIPProcessor, CLIPModel, Qwen2_5_VLForConditionalGeneration, AutoProcessor, BitsAndBytesConfig
from qwen_vl_utils import process_vision_info # Make sure this file/function is available
...
class VLMContextExtractor:
    ...
    def extract_context_from_image(self, image_pil):
        ...
        try:
            ...
            image_inputs, video_inputs = process_vision_info(messages) # From qwen_vl_utils
            ...
            return output_text[0].strip() if output_text and output_text[0].strip() else None
...
class EnhancedCLIPAdapter(nn.Module):
    def __init__(self, clip_model_name=config.CLIP_MODEL_NAME,
                 ...
                 bottleneck_dim=config.BOTTLENECK_DIM, device=config.DEVICE,
        ...
        with torch.no_grad():
            ...
            self.text_feature_dim = self.image_feature_dim # CLIP text and image features have the same dim

        # Initialize Adapters
        self.visual_adapter = VisualAdapter(self.image_feature_dim, bottleneck_dim).to(self.device)
        self.text_adapter = TextAdapter(self.text_feature_dim, bottleneck_dim).to(self.device)
        self.context_adapter = ContextAdapter(self.text_feature_dim, bottleneck_dim).to(self.device) # Assuming context features have same dim as CLIP text

        self.alpha = alpha
        self.beta = beta
        self.gamma = gamma

        self.vlm_context_extractor = vlm_context_extractor if vlm_context_extractor else VLMContextExtractor(device=self.device)
    ...
    def encode_emotion_descriptions(self, emotions=config.EMOTIONS):
        ...
        self.emotion_descriptions = {emotion: [f"A person expressing {emotion}"] for emotion in emotions}
        self.original_emotion_text_features = {}
        ...
        with torch.no_grad():
            for emotion, descriptions in self.emotion_descriptions.items():
                ...
                if emotion_features_list:
                    ...
                    self.original_emotion_text_features[emotion] = torch.zeros(1, self.text_feature_dim).to(self.device)
    ...
    def update_emotion_embeddings(self):
        ...
        if self.emotion_embedding_tensor is None:
            print("Warning: Original emotion embeddings not encoded. Call encode_emotion_descriptions first.")
            return

        with torch.no_grad(): # Adapters are trained, but this update uses their current state
            original_text_features = self.emotion_embedding_tensor.clone().detach()
            adapter_output = self.text_adapter(original_text_features)
            adapted_feature = self.beta * adapter_output + (1 - self.beta) * original_text_features
            self.adapted_emotion_embedding_tensor = adapted_feature / adapted_feature.norm(dim=-1, keepdim=True)


    def forward(self, pixel_values, context_features=None, use_adapters_for_training=True):
        """
        ...
        if context_features is not None and context_features.nelement() > 0 : # Check if tensor is not empty
            ...
            if context_features.shape[-1] != self.text_feature_dim:
                ...
                print(f"Warning: Context feature dimension mismatch. Expected {self.text_feature_dim}, got {context_features.shape[-1]}. Skipping context.")
                combined_features = final_image_features
            else:
                adapted_context_features = self.context_adapter(context_features)
                final_context_features = self.gamma * adapted_context_features + (1 - self.gamma) * context_features
                final_context_features = final_context_features / final_context_features.norm(dim=-1, keepdim=True)
                combined_features = (final_image_features + final_context_features) / 2.0 # Average fusion
                combined_features = combined_features / combined_features.norm(dim=-1, keepdim=True)
        else:
            combined_features = final_image_features

        # Text Features (Emotion Embeddings)
        if self.training or not hasattr(self, 'adapted_emotion_embedding_tensor') or self.adapted_emotion_embedding_tensor is None:
            ...
            original_text_features = self.emotion_embedding_tensor.clone().detach() # Start with original
            adapted_text_features_output = self.text_adapter(original_text_features)
            final_text_features = self.beta * adapted_text_features_output + (1 - self.beta) * original_text_features
    ...
    def predict_probs(self, pixel_values, context_features=None):
        ...
        with torch.no_grad():
            logits = self.forward(pixel_values, context_features, use_adapters_for_training=False)
    ...
    def print_model_structure(self):
        ...
        if self.vlm_context_extractor:
            ...
            vlm_params = sum(p.numel() for p in self.vlm_context_extractor.vlm_model.parameters())
            ...
            if hasattr(self.vlm_context_extractor, 'clip_model') and self.vlm_context_extractor.clip_model is not self.model:
                vlm_clip_params = sum(p.numel() for p in self.vlm_context_extractor.clip_model.parameters())
                total_params += vlm_clip_params
                print(f"VLM's CLIP Encoder Parameters: {vlm_clip_params:,}")


        adapter_params_total = 0
        for name, adapter in [("Visual", self.visual_adapter), ("Text", self.text_adapter), ("Context", self.context_adapter)]:
